...
function handleCreate(name: string, opts: { description: string; json?: boolean }): void {
...
function handleList(opts: { json?: boolean }): void {
  const isJson = opts.json ?? false;
  const ctx = guardNotInitialized({}, isJson);
  if (!ctx) return;
...
function handleValidate(name: string | undefined, opts: { json?: boolean }): void {
  const isJson = opts.json ?? false;
  const ctx = guardNotInitialized({}, isJson);
  if (!ctx) return;
  ...
  if (name) {
    validateSingleSkill(skillsDir, name, isJson);
  } else {
    validateAllSkills(skillsDir, isJson);
  }
}

function validateSingleSkill(skillsDir: string, name: string, isJson: boolean): void {
...
function validateAllSkills(skillsDir: string, isJson: boolean): void {
  const skills = listSkillFiles(skillsDir);
  let validCount = 0;
  let invalidCount = 0;
...
export function skillCommand(): Command {
  ...
  cmd
    ...
    .action((name: string, opts: { description: string; json?: boolean }) => handleCreate(name, opts));

  cmd
    .command("list")
    .description("List all skill files")
    .option("--json", "Output as JSON")
    .action((opts: { json?: boolean }) => handleList(opts));

  cmd
    .command("validate [name]")
    .description("Validate skill file(s) — all skills if no name given")
    .option("--json", "Output as JSON")
    .action((name: string | undefined, opts: { json?: boolean }) => handleValidate(name, opts));
