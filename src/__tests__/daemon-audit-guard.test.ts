...
describe("daemon never triggers the heavy verify:all path", () => {
  ...
  it("auditHealth is called with fullSweep-equivalent behavior disabled inside the daemon", () => {
    ...
    // auditHealth(projectRoot, shitennoDir, level, changedFiles?) — must audit the PROJECT, not .shitenno
    expect(timersSource).toMatch(/auditHealth\(ctx\.projectRoot, ctx\.shitennoDir, level, changedFiles\)/);
  });
});
